            ...
            try
            {
                ...
                foreach (SpiraSoapService.RemoteDataMapping projectMapping in projectMappings)
                {
                    ...
                    foreach (SpiraSoapService.RemoteIncident remoteIncident in incidentList)
                    {
                        try
                        {
                            ProcessIncident(projectId, spiraImportExport, remoteIncident, newIncidentMappings, newReleaseMappings, oldReleaseMappings, incidentCustomPropertyMappingList, incidentCustomPropertyValueMappingList, incidentCustomProperties, incidentMappings, jiraProject, jiraManager, productName, incidentSeverityMappings, incidentPriorityMappings, incidentStatusMappings, incidentTypeMappings, userMappings, releaseMappings, incidentComponentMappings);
                        }
                        catch (Exception exception)
                        {
                            //Log and continue execution
